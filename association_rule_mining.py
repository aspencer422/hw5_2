...
# SPECIFICATION: association_rule_mining
...
#Example:
#{'Pencil', 'Meat', 'Bagel', 'Wine', 'Diaper', 'Milk', 'Bread', 'Cheese', 'Eggs'}
new_list=[]

for i in range(len(arr)):
    temp=[]
    if 'Pencil' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)

    if 'Meat' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)

    if 'Bagel' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)

    if 'Wine' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)

    if 'Diaper' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)

    if 'Milk' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)

    if 'Bread' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)

    if 'Cheese' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)

    if 'Eggs' not in arr[i]:
        temp.append(1)
    else:
        temp.append(0)
...
for i in range(len(arr2)):
    print("Support: " + str(arr2[i][2]))
    print("Confidence: " + str(arr2[i][3]))
